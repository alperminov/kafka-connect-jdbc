...
/**
 * JdbcSourceTaskExt is a Kafka Connect SourceTask implementation that reads from JDBC databases and
 * generates Kafka Connect records, with extended config.
 */
public class JdbcSourceTaskExt extends SourceTask {

  private static final Logger log = LoggerFactory.getLogger(JdbcSourceTaskExt.class);

  private Time time;
  private JdbcSourceTaskConfigExt config;
  private DatabaseDialect dialect;
  private CachedConnectionProvider cachedConnectionProvider;
  private PriorityQueue<TableQuerier> tableQueue = new PriorityQueue<TableQuerier>();
  private final AtomicBoolean running = new AtomicBoolean(false);

  public JdbcSourceTaskExt() {
    this.time = new SystemTime();
  }

  public JdbcSourceTaskExt(Time time) {
    this.time = time;
  }
  ...
  public void start(Map<String, String> properties) {
    ...
    try {
      config = new JdbcSourceTaskConfigExt(properties);
    } catch (ConfigException e) {
      throw new ConnectException("Couldn't start JdbcSourceTask due to configuration error", e);
    }

    final String url = config.getString(JdbcSourceTaskConfigExt.CONNECTION_URL_CONFIG);
    final int maxConnAttempts = config.getInt(JdbcSourceTaskConfigExt.CONNECTION_ATTEMPTS_CONFIG);
    final long retryBackoff = config.getLong(JdbcSourceTaskConfigExt.CONNECTION_BACKOFF_CONFIG);

    final String dialectName = config.getString(JdbcSourceTaskConfigExt.DIALECT_NAME_CONFIG);
    ...
    List<String> tables = config.getList(JdbcSourceTaskConfigExt.TABLES_CONFIG);
    String query = config.getString(JdbcSourceTaskConfigExt.QUERY_CONFIG);
    ...
    String mode = config.getString(JdbcSourceTaskConfigExt.MODE_CONFIG);
    //used only in table mode
    Map<String, List<Map<String, String>>> partitionsByTableFqn = new HashMap<>();
    Map<Map<String, String>, Map<String, Object>> offsets = null;
    if (mode.equals(JdbcSourceTaskConfigExt.MODE_INCREMENTING)
            || mode.equals(JdbcSourceTaskConfigExt.MODE_TIMESTAMP)
            || mode.equals(JdbcSourceTaskConfigExt.MODE_TIMESTAMP_INCREMENTING)) {
    ...
    String incrementingColumn
            = config.getString(JdbcSourceTaskConfigExt.INCREMENTING_COLUMN_NAME_CONFIG);
    List<String> timestampColumns
            = config.getList(JdbcSourceTaskConfigExt.TIMESTAMP_COLUMN_NAME_CONFIG);
    Long timestampDelayInterval
            = config.getLong(JdbcSourceTaskConfigExt.TIMESTAMP_DELAY_INTERVAL_MS_CONFIG);
    boolean validateNonNulls
            = config.getBoolean(JdbcSourceTaskConfigExt.VALIDATE_NON_NULL_CONFIG);
    ...
    for (String tableOrQuery : tablesOrQuery) {
      ...
      String topicPrefix = config.getString(JdbcSourceTaskConfigExt.TOPIC_PREFIX_CONFIG);

      if (mode.equals(JdbcSourceTaskConfigExt.MODE_BULK)) {
        tableQueue.add(
                new BulkTableQuerier(dialect, queryMode, tableOrQuery, topicPrefix)
        );
      } else if (mode.equals(JdbcSourceTaskConfigExt.MODE_INCREMENTING)) {
        tableQueue.add(
      ...
      } else if (mode.equals(JdbcSourceTaskConfigExt.MODE_TIMESTAMP)) {
        tableQueue.add(
                new TimestampIncrementingTableQuerier(
                        dialect,
                        queryMode,
      ...
      } else if (mode.endsWith(JdbcSourceTaskConfigExt.MODE_TIMESTAMP_INCREMENTING)) {
        tableQueue.add(
                new TimestampIncrementingTableQuerier(
                        dialect,
                        queryMode,
  ...
  public List<SourceRecord> poll() throws InterruptedException {
    ...
    final boolean autoStop = config.getBoolean(JdbcSourceTaskConfigExt.AUTO_STOP_CONFIG);
    final String mode = config.getString(JdbcSourceTaskConfigExt.MODE_CONFIG);

    while (running.get()) {
      final TableQuerier querier = tableQueue.peek();

      if (!querier.querying()) {
        // Stop task when querying finished
        ...
        final long nextUpdate = querier.getLastUpdate()
                + config.getInt(JdbcSourceTaskConfigExt.POLL_INTERVAL_MS_CONFIG);
      ...
      try {
        ...
        int batchMaxRows = config.getInt(JdbcSourceTaskConfigExt.BATCH_MAX_ROWS_CONFIG);
  ...
  private void validateNonNullable(
    ...
    try {
      ...
      if ((incrementalMode.equals(JdbcSourceTaskConfigExt.MODE_INCREMENTING)
              || incrementalMode.equals(JdbcSourceTaskConfigExt.MODE_TIMESTAMP_INCREMENTING))
      ...
      if ((incrementalMode.equals(JdbcSourceTaskConfigExt.MODE_TIMESTAMP)
              || incrementalMode.equals(JdbcSourceTaskConfigExt.MODE_TIMESTAMP_INCREMENTING))
